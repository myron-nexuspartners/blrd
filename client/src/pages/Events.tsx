...
import { ArrowUpDown, MapPin, ExternalLink, Calendar, Tag, Monitor, ChevronUp, ChevronDown } from "lucide-react";

const EVENTS_DATA = [
  {
    id: 1,
    name: "BlerdCon 2025",
    description: "The DMV's premier convention celebrating Black geek culture. Three days of panels, gaming tournaments, cosplay competitions, artist alley, and community building. Featuring special guests from gaming, comics, film, and television.",
    type: "convention",
    category: "multi",
  ...
  },
  {
    ...
    description: "The world's largest and longest-running fighting game tournament. Featuring Tekken 8, Street Fighter 6, Mortal Kombat 1, and more. Watch the best players in the world compete for the championship.",
    type: "tournament",
    category: "gaming",
  ...
  },
  {
    ...
    description: "The world's largest comics and pop culture convention. Four days of panels, exclusives, celebrity appearances, and the most anticipated announcements in entertainment.",
    type: "convention",
    category: "comics",
  ...
  },
  {
    ...
    description: "A curated virtual film festival celebrating Afrofuturist cinema from around the world. Featuring feature films, short films, and panel discussions with filmmakers.",
    type: "screening",
    category: "film",
    location: "Online",
    isVirtual: true,
  ...
  },
  {
    id: 5,
    name: "PAX West 2025",
    description: "One of the largest gaming festivals in the world. Four days of game demos, panels, tournaments, and community events in Seattle.",
    type: "convention",
    category: "gaming",
    location: "Seattle, WA",
    isVirtual: false,
  ...
  },
  {
    ...
    description: "Community watch party celebrating the legacy of Black Panther. Featuring pre-show discussion, live commentary, and post-film panel with local creators.",
    type: "watch-party",
    category: "film",
  ...
  },
  {
    ...
    description: "A hands-on workshop for aspiring game developers focused on creating authentic, culturally resonant narratives. Featuring industry professionals as mentors.",
    type: "workshop",
    category: "gaming",
    location: "Online",
    isVirtual: true,
  ...
  },
  {
    ...
    description: "One of the largest pop culture events in North America. Comics, gaming, anime, film, TV, and more — all under one roof in New York City.",
    type: "convention",
    category: "multi",
  ...
  },
  {
    ...
    description: "A live streaming tournament showcasing the best indie games of 2025. Watch top streamers compete and discover your next favorite game.",
    type: "virtual",
    category: "gaming",
  ...
  },
  {
    ...
    description: "North America's largest anime convention. Featuring industry panels, voice actor appearances, cosplay, and exclusive merchandise.",
    type: "convention",
    category: "culture",
...
function EventModal({ event, onClose }: { event: typeof EVENTS_DATA[0]; onClose: () => void }) {
        ...
        <div className="relative h-48">
          ...
          <div className="absolute bottom-3 left-4">
            <div className="flex gap-2 mb-1">
              ...
              <span
                className="blrd-tag"
                style={{ background: "rgba(255,255,255,0.1)", color: "var(--blrd-white)", border: "1px solid rgba(255,255,255,0.2)" }}
              >
...
export default function Events() {
  ...
  const sorted = [...EVENTS_DATA]
    .filter((e) => (typeFilter === "all" || e.type === typeFilter) && (catFilter === "all" || e.category === catFilter))
    .sort((a, b) => {
      let valA: string | number | Date = a[sortKey] as any;
      let valB: string | number | Date = b[sortKey] as any;
  ...
  const uniqueTypes = Array.from(new Set(EVENTS_DATA.map((e) => e.type)));
  const uniqueCats = Array.from(new Set(EVENTS_DATA.map((e) => e.category)));
    ...
    <Layout showSidebar={false}>
      ...
      >
        <div className="container">
          ...
          <h1 className="font-display font-black mb-2" style={{ fontSize: "clamp(1.8rem, 4vw, 2.8rem)", color: "var(--blrd-white)" }}>
            Upcoming Events
          </h1>
      ...
      <div className="container py-8">
        ...
        <div className="flex flex-col sm:flex-row gap-3 mb-6 flex-wrap">
          <div className="flex gap-2 flex-wrap">
            <button
              ...
              style={{
                background: typeFilter === "all" ? "var(--blrd-cyan)" : "var(--blrd-dark-2)",
                borderColor: typeFilter === "all" ? "var(--blrd-cyan)" : "var(--blrd-border)",
                color: typeFilter === "all" ? "var(--blrd-black)" : "var(--blrd-gray-light)",
              }}
            >
              All Types
            </button>
            {uniqueTypes.map((t) => (
              <button
                ...
                className="px-3 py-1.5 text-xs rounded border transition-all font-ui font-semibold"
                style={{
                  background: typeFilter === t ? "var(--blrd-cyan)" : "var(--blrd-dark-2)",
                  borderColor: typeFilter === t ? "var(--blrd-cyan)" : "var(--blrd-border)",
                  color: typeFilter === t ? "var(--blrd-black)" : "var(--blrd-gray-light)",
                }}
              >
                {TYPE_LABELS[t]}
              </button>
          ...
          <div className="flex gap-1 ml-auto">
            {(["cards", "table"] as const).map((mode) => (
              <button
                ...
                className="px-3 py-1.5 text-xs rounded border transition-all font-ui font-semibold capitalize"
                style={{
                  background: viewMode === mode ? "var(--blrd-dark-3)" : "transparent",
                  borderColor: viewMode === mode ? "var(--blrd-cyan)" : "var(--blrd-border)",
                  color: viewMode === mode ? "var(--blrd-cyan)" : "var(--blrd-gray-light)",
                }}
              >
                {mode}
              </button>
        ...
        <div className="section-header">
          <h2>Events</h2>
          <span className="text-xs font-ui" style={{ color: "var(--blrd-gray)" }}>{sorted.length} events</span>
        </div>
            ...
            {sorted.map((event) => (
              ...
              >
                ...
                <div className="p-4">
                  ...
                  <div className="flex items-center gap-3 text-xs font-ui" style={{ color: "var(--blrd-gray)" }}>
                    <span className="flex items-center gap-1"><MapPin size={10} />{event.location}</span>
                    <span className="ml-auto" style={{ color: "#00e676" }}>{event.price}</span>
                  </div>
                </div>
              </div>
            ))}
        ...
        {/* Table View */}
        {viewMode === "table" && (
          <div className="overflow-x-auto rounded" style={{ border: "1px solid var(--blrd-border)" }}>
            <table className="w-full text-sm">
              <thead>
                <tr style={{ background: "var(--blrd-dark-2)", borderBottom: "1px solid var(--blrd-border)" }}>
                  {(
                    [
                    ...
                    <th
                      ...
                      style={{ color: "var(--blrd-gray)", fontFamily: "Rajdhani, sans-serif", fontSize: "0.75rem", letterSpacing: "0.08em", textTransform: "uppercase" }}
                  ...
                  <th className="text-left px-4 py-3" style={{ color: "var(--blrd-gray)", fontFamily: "Rajdhani, sans-serif", fontSize: "0.75rem", letterSpacing: "0.08em", textTransform: "uppercase" }}>
                    Details
                  </th>
                </tr>
              </thead>
              <tbody>
                {sorted.map((event, i) => (
                  <tr
                    key={event.id}
                    style={{
                      background: i % 2 === 0 ? "var(--blrd-dark)" : "var(--blrd-dark-2)",
                      borderBottom: "1px solid var(--blrd-border)",
                    }}
                  >
                    <td className="px-4 py-3">
                      <div className="font-semibold text-sm" style={{ color: "var(--blrd-white)", fontFamily: "Inter, sans-serif" }}>
                        {event.name}
                      </div>
                      {event.isVirtual && (
                    ...
                    <td className="px-4 py-3 text-xs font-ui whitespace-nowrap" style={{ color: "var(--blrd-gray-light)" }}>
                      {formatDate(event.startDate)}
                      {event.endDate && event.endDate.toDateString() !== event.startDate.toDateString() && (
                        <div style={{ color: "var(--blrd-gray)" }}>— {formatDate(event.endDate)}</div>
                      )}
                    </td>
                    <td className="px-4 py-3">
                      <span
                        className="blrd-tag"
                        style={{ background: "rgba(255,255,255,0.08)", color: "var(--blrd-gray-light)", border: "1px solid var(--blrd-border)" }}
                      >
                        {TYPE_LABELS[event.type]}
                      </span>
                    </td>
                    ...
                    <td className="px-4 py-3 text-xs font-ui" style={{ color: "var(--blrd-gray-light)" }}>
                      <span className="flex items-center gap-1">
        ...
        >
          ...
          <Link href="/contact">
            <button
              className="px-5 py-2 rounded font-ui font-bold text-xs tracking-wider transition-all hover:brightness-110"
              style={{ background: "var(--blrd-cyan)", color: "var(--blrd-black)", letterSpacing: "0.08em" }}
            >
              Submit an Event →
            </button>
